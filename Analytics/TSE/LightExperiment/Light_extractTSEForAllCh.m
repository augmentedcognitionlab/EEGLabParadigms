...
EEG = pop_loadset('filename',[files(n).name],'filepath','C:\\Users\\Experiments\\Desktop\\Experiments\\MadsLight\\FinalData\\raw\\processed\\NoBadCh\\ica\\ICaPruned\\Epoched\\Green');
...
%in the difErsp the difference maps are stoGreen, be aware that the difErsp
%are cells and that it is the third matrix in the cell that is the
%comparrisson. 
    figure; 
    newTitle = electrodeInfo.labels; 
    newTitle = strcat(newTitle,'Green');
   ...
   % end
    ...
    [GreenerspStat(n,e,:,:),itc,powbase,times,freqs,erspboot,itcboot] = newtimef( EEG.data(e,:,:), EEG.pnts, [EEG.xmin EEG.xmax]*1000, EEG.srate, [3 0.5],  'baseline',[-5000 0], 'alpha',0.05, 'freqs', [0.5 50], 'title', newTitle);
   ...
   nameOfFile = strcat(nameOfFile,'Green');
    %if mod(m,2) == 0
    ...
    [Greenersp(n,e,:,:),itc,powbase,times,freqs,erspboot,itcboot] = newtimef( EEG.data(e,:,:), EEG.pnts, [EEG.xmin EEG.xmax]*1000, EEG.srate, [3 0.5],  'baseline',[-5000 0], 'freqs', [0.5 50], 'title', newTitle);
            pop_newtimef( EEG, e, 2, [-5000  29996], [3         0.5] , 'topovec', 2, 'elocs', EEG.chanlocs, 'chaninfo', EEG.chaninfo, 'caption', electrodeInfo.labels, 'baseline',[-5000 0], 'freqs', [0.5 50], 'plotphase', 'off', 'padratio', 1);
   ...
   % nameOfFile = strcat(nameOfFile,'Electrode');
   % nameOfFile = strcat(nameOfFile,electrodeInfo.labels);
   % nameOfFile = strcat(nameOfFile,'CompaGreenAbsWithFig');
   % nameOfFile = strcat(nameOfFile,'.fig');
   % saveas(gcf,nameOfFile)
   %     close all
   %     end
   ...
   % comparissons = sessions/2; 
    electrodes = 16; % the number of electrodes 
    xPos = 793;
    yPos = 200; 
  ...
  %  end
   ...
   % end
    ...
    for e = 1:electrodes 
        for x = 1:xPos
            for y = 1:yPos
        averageERSPGreen(e,x,y) = mean(Greenersp(:,e,x,y));
        averageERSPGreenStat(e,x,y) = mean(GreenerspStat(:,e,x,y)); 
       % averageERSPabs(e,x,y) = mean(absERSP(:,e,x,y));
       % averageERSPabsStat(e,x,y) = mean(absERSPStat(:,e,x,y)); 
        
        stdERSPGreen(e,x,y) = std(Greenersp(:,e,x,y));
        stdERSPGreenStat(e,x,y) = std(GreenerspStat(:,e,x,y)); 
