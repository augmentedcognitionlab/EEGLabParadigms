...
EEG = pop_loadset('filename',[files(n).name],'filepath','D:\\ArtInHosp\\EEGLAB_Ready\\processed\\NoBadCh\\ica\\Cleaned\\epoched\\AbsFig\\');
...
%comparrisson. 
    ...
    [erspStat(n,e,:,:),itc,powbase,times,freqs,erspboot,itcboot] = newtimef( EEG.data(e,:,:), EEG.pnts, [EEG.xmin EEG.xmax]*1000, EEG.srate, [3 0.5],  'baseline',[-5000 - 0], 'alpha',0.05, 'freqs', [0.5 50], 'title', newTitle);
...
if(mod(m,2) == 0)
    ...
    for e = 1:electrodes; 
        for x = 1:xPos;
            for y = 1:yPos
        averageERSPfig(e,x,y) = mean(figERSP(:,e,x,y));
        averageERSPfigStat(e,x,y) = mean(figERSPStat(:,e,x,y)); 
        averageERSPabs(e,x,y) = mean(absERSP(:,e,x,y));
        averageERSPabsStat(e,x,y) = mean(absERSPStat(:,e,x,y)); 
        
        stdERSPfig(e,x,y) = std(figERSP(:,e,x,y));
        stdERSPfigStat(e,x,y) = std(figERSPStat(:,e,x,y)); 
